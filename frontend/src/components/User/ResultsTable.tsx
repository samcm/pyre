...
export function ResultsTable({ username }: ResultsTableProps) {
  const { data, isLoading, error, refetch } = useResults(username);
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-text-muted mt-1 text-sm/6">{total} resolved positions</p>
      </div>
      <Card padding={false}>
